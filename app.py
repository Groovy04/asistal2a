...
def sales_planned_entry(): #Add user - ALSO TO DATABASE!!!
    ...
    if form.validate_on_submit():
        ...
        if unique_month is None:
            ...
            month_order ={"Oca": 1, "Şub": 2, "Mar": 3, "Nis": 4, "May": 5, "Haz": 6, "Tem": 7, 
            ...
            db.session.commit()
            flash("Kayıt başarı ile kaydedildi! Teşekkürler!", "success")
        ...
        #form.email_field.data = ""
            ...
            flash("Bu ayın kaydı daha önce yapılmış! Lütfen kaydetmek için farklı bir ay seçiniz veya mevcut ay kaydını güncellemeyi deneyiniz!", 'error')
...
def update_planned(id):
    ...
    if request.method == "POST": #burayı daha iyi anlamalıyız!!! 
        ...
        try:
            db.session.commit()
            flash("Kayıt güncelleme işlemi başarılı. Teşekkürler.", "success")
            all_records = SALESMTPLANNED_db.query.order_by(SALESMTPLANNED_db.year_planned_db, SALESMTPLANNED_db.month_order_db)
            return render_template("update_planned_entry.html", form=form, entry_to_update=entry_to_update, id=id, all_records=all_records)
            
        except:
            db.session.commit()
            flash("Kayıt güncelleme işlemi BAŞARISIZ. Lütfen IT Yöneticisi ile temasa geçiniz.", "success")
            all_records = SALESMTPLANNED_db.query.order_by(SALESMTPLANNED_db.year_planned_db, SALESMTPLANNED_db.month_order_db)
            return render_template("update_planned_entry.html", form=form, entry_to_update=entry_to_update, id=id, all_records=all_records)
    else:
...
def delete_planned(id):
    ...
    try:
        ...
        db.session.commit()
        flash("Kayıt silindi. Teşekkürler.", "success")
        ...
        flash("İşlemde bir hata oldu. Lütfen IT Yönetici ile temasa geçiniz.", 'error')
        return  render_template("sales_planned_entry.html", form=form, name=name, all_records=all_records)
...
def customer_form1(): #Add user - ALSO TO DATABASE!!!
    ...
    if form.validate_on_submit():
        ...
        if unique_company is None:
            ...
            flash("Aynı isimli bir firma daha önce kaydedilmiş! Lütfen başka bir isimle bir firma kaydediniz veya mevcut müşteri bilgilerini güncelleyiniz!", 'error')
    
    elif not form.validate_on_submit():
            print("There is a problem in filling the form. Missing field.")
            flash("Lütfen 1-Şirket İsmi, 2-Müşteri Personeli -1 Adı Soyadı ve 3- Müşteri Personeli -1 E-mail alanlarının doldurulduğundan emin olunuz.", 'error')
...
def customer_search():
    ...
    if form.validate_on_submit():
        ...
        if not filtered_companies:
            flash("Bu müşteri bulunamadı. Lütfen şirket ismini kontrol edip tekrar deneyiniz.", "error")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and password_check:
            ...
            flash('Sisteme başarı ile giriş yaptınız. İyi günler dileriz.', 'success')
            return redirect(url_for('index'))
        else:
            flash('Sisteme girişinizde bir sorun oluştu. Lütfen giriş isminizi/emailinizi ve şifrenizi kontrol ediniz.', 'error')
...
def logout():
    ...
    flash('Sistemden başarı ile çıkış yaptınız. İyi günler dileriz.', 'success')
    return redirect(url_for('index'))
...
def performance_graph_2021():
    ...
    f = figure(x_range=filtered_planned_data["month_unique_db"], title ="Toplam Planlanan ve Gerçekleşen Satış Hacmi - MT")
    ...
    f2 = figure(x_range=filtered_planned_data["month_unique_db"], title ="Topl. Plan. ve Gerçek. Sat. Hacmi MF Fazı - MT")
    ...
    f3 = figure(x_range=filtered_planned_data["month_unique_db"], title ="Topl. Plan. ve Gerçek. Sat. Hacmi Anodize Fazı - MT")
    ...
    f4 = figure(x_range=filtered_planned_data["month_unique_db"], title ="Topl. Plan. ve Gerçek. Sat. Hacmi Powder Coating Fazı - MT")
    ...
    f5 = figure(x_range=filtered_planned_data["month_unique_db"], title ="Topl. Plan. ve Gerçek. Sat. Hacmi Wood Finish Fazı - MT")
    ...
    f6 = figure(x_range=filtered_planned_data["month_unique_db"], title ="Topl. Plan. ve Gerçek. Sat. Hacmi Crimping Fazı - MT")
    ...
    f7 = figure(x_range=filtered_planned_data["month_unique_db"], title ="EURO Bazlı Planlanan ve Gerçekleşen Toplam Satış Tutarı")
    ...
    f8 = figure(x_range=filtered_planned_data["month_unique_db"], title ="Planlanan ve Gerçekleşen Toplam Yeni Müşteri Sayısı")
